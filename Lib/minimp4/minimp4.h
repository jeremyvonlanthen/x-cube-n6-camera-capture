...
/* --- Custom allocator hooks (added for STM32N6 integration) ---
 * Define MINIMP4_MALLOC/REALLOC/FREE before including this header to route
 * all internal allocations to a custom pool (no libc heap on this target). */
#ifndef MINIMP4_MALLOC
#define MINIMP4_MALLOC(s)     malloc(s)
#define MINIMP4_REALLOC(p, s) realloc(p, s)
#define MINIMP4_FREE(p)       free(p)
#define MINIMP4_STRDUP(s)     strdup(s)
#endif
...
#ifndef MINIMP4_TRANSCODE_SPS_ID
...
*   Return pointer to internal mp4 memory, it must not be MINIMP4_FREE()-ed
*
...
static int minimp4_vector_init(minimp4_vector_t *h, int capacity)
{
    h->bytes = 0;
    h->capacity = capacity;
    h->data = capacity ? (unsigned char *)MINIMP4_MALLOC(capacity) : NULL;
...
static int minimp4_vector_grow(minimp4_vector_t *h, int bytes)
{
    ...
    p = MINIMP4_REALLOC(h->data, new_size);
    if (!p)
        return 0;
    ...
    int (*write_callback)(int64_t offset, const void *buffer, size_t size, void *token))
{
    if (write_callback(0, box_ftyp, sizeof(box_ftyp), token)) // Write fixed header: 'ftyp' box
        return 0;
    MP4E_mux_t *mux = (MP4E_mux_t*)MINIMP4_MALLOC(sizeof(MP4E_mux_t));
    if (!mux)
        return mux;
...
int MP4E_set_text_comment(MP4E_mux_t *mux, const char *comment)
{
    if (!mux || !comment)
        return MP4E_STATUS_BAD_ARGUMENTS;
    if (mux->text_comment)
        MINIMP4_FREE(mux->text_comment);
    mux->text_comment = MINIMP4_STRDUP(comment);
    if (!mux->text_comment)
        return MP4E_STATUS_NO_MEMORY;
    return MP4E_STATUS_OK;
}
...
#define TRACK_HEADER_BYTES 512
    ...
    base = (unsigned char*)MINIMP4_MALLOC(index_bytes);
    if (!base)
        return MP4E_STATUS_NO_MEMORY;
    p = base;
...
void mp4_h26x_write_close(mp4_h26x_writer_t *h)
{
    ...
    {
        if (p->sps_cache[i])
            MINIMP4_FREE(p->sps_cache[i]);
    }
    for (i = 0; i < MINIMP4_MAX_PPS; i++)
    {
        if (p->pps_cache[i])
            MINIMP4_FREE(p->pps_cache[i]);
    }
#endif
    memset(h, 0, sizeof(*h));
}
...
static int mp4_h265_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal, int sizeof_nal, unsigned timeStamp90kHz_next)
{
    ...
    default:
        if (h->need_vps || h->need_sps || h->need_pps || h->need_idr)
            return MP4E_STATUS_BAD_ARGUMENTS;
        {
            unsigned char *tmp = (unsigned char *)MINIMP4_MALLOC(4 + sizeof_nal);
            if (!tmp)
                return MP4E_STATUS_NO_MEMORY;
...
int mp4_h26x_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal, int length, unsigned timeStamp90kHz_next)
{
    ...
    for (;;nal++)
    {
        ...
        nal1 = (unsigned char *)MINIMP4_MALLOC(sizeof_nal*17/16 + 32);
        if (!nal1)
            return MP4E_STATUS_NO_MEMORY;
        nal2 = (unsigned char *)MINIMP4_MALLOC(sizeof_nal*17/16 + 32);
        if (!nal2)
        {
            MINIMP4_FREE(nal1);
            return MP4E_STATUS_NO_MEMORY;
...
#define MALLOC(t, p, size) p = (t)MINIMP4_MALLOC(size); if (!(p)) { ERROR("out of memory"); }
        ...
        case BOX_stts:
            {
                ...
                for (i = 0; i < count; i++)
                {
                    ...
                    {
                        ts_count = k + sc;
                        tr->timestamp = (unsigned int*)MINIMP4_REALLOC(tr->timestamp, ts_count * sizeof(unsigned));
                        tr->duration  = (unsigned int*)MINIMP4_REALLOC(tr->duration,  ts_count * sizeof(unsigned));
                    }
                    for (j = 0; j < sc; j++)
                    {
                        tr->duration[k] = d;
...
//         case BOX_svc1:   - no test
        ...
        case BOX_avcC:  // AVCDecoderConfigurationRecord()
            ...
            tr->dsi = (unsigned char*)MINIMP4_MALLOC((size_t)box_bytes);
        ...
        {
            // New track found: allocate memory using MINIMP4_REALLOC()
            ...
            void *mem = MINIMP4_REALLOC(mp4->track, (mp4->track_count + 1)*sizeof(MP4D_track_t));
            if (!mem)
            {
...
#define FREE(x) if (x) {MINIMP4_FREE(x); x = NULL;}
